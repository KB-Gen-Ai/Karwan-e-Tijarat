    ...
    with st.form("profile_form"):
        ...
        if submitted:
            if not (name and email and phone):
                ...
                st.markdown(download_button(generate_pdf((0, name, email, phone, city, profession, skills, help_offer, help_seek)),
                                           f"Karwan_Profile_{name}.pdf", "📄 Download PDF"), unsafe_allow_html=True)

elif menu == "Search Directory":
    st.subheader("🔍 Search Profiles")
    search_term = st.text_input("Search by Name, City, or Profession")
    if search_term:
        query = f"""
        ...
        if results:
            for r in results:
                ...
                st.markdown(f"💼 **Profession:** {r[5]}")
                st.markdown(f"🛠️ **Skills:** {r[6]}")
                st.markdown(f"🤝 **Can Help With:** {r[7]}")
                st.markdown(f"🙏 **Needs Help With:** {r[8]}")
